...
Characteristic.prototype.stream = function() {
  var dup = duplexify();
  var characteristic = this;

  var onNotify = function(state){
  ...
  if(!characteristic.subscribed){
    characteristic.notify(true, onNotify); //todo handle this error?
  }else{
    onNotify(characteristic.subscribed);
  }
